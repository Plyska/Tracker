 ...
 *
 * **Синхронізовано вручну з `CRISIS_RESOURCES` у `ai.prompts.ts`.** Зв'язок навмисний: кризовий
 * текст приходить готовим рядком, бо контакти мусять бути дослівні, — але тоді номер, якого тут
 * немає, просто лишиться простим текстом. Міняєш номери на сервері — міняй і тут.
 *
 * Довгий номер стоїть першим у чергуванні: інакше коротші альтернативи відкусили б його шматками.
 */
const PHONE_SPLIT = /(0 800 210 160|\b112\b|\b103\b)/g;
const IS_PHONE = /^(0 800 210 160|112|103)$/;

/** `tel:` не терпить пробілів усередині номера. */
const telHref = (n: string): string => `tel:${n.replace(/\s/g, "")}`;
